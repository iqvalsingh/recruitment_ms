...
public interface DepartmentRepository
	...
	@Query("SELECT new com.ism.job.recruitment.dto.ChartDTO(d.name, COUNT(v.id)) FROM Department d LEFT JOIN d.vacancies v GROUP BY d.name")
	List<ChartDTO> getDepartmentWithVacancyCount();

	@Query("SELECT new com.ism.job.recruitment.dto.ChartDTO(d.name, COALESCE(COUNT(v.id), 0)) " + "FROM Department d "
