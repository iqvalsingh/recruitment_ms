...
public interface PositionRepository extends JpaRepository<Position, Integer>, DataTablesRepository<Position, Integer> {
	...
	@Query("SELECT new com.ism.job.recruitment.dto.ChartDTO(p.name, COUNT(v.id)) FROM Position p LEFT JOIN p.vacancies v GROUP BY p.name")
	List<ChartDTO> getPositionWithVacancyCount();

	@Query("SELECT new com.ism.job.recruitment.dto.ChartDTO(p.name, COALESCE(COUNT(v.id), 0)) " + "FROM Position p "
	...
	@Query("SELECT new com.ism.job.recruitment.dto.ChartDTO(p.name, COUNT(c.id)) " + "FROM Position p "
			+ "LEFT JOIN p.vacancies v LEFT JOIN v.candidates c " + "GROUP BY p.name")
	List<ChartDTO> getCandidateCountByPosition();

	@Query("SELECT new com.ism.job.recruitment.dto.ChartDTO(p.name, COALESCE(COUNT(c.id), 0)) " + "FROM Position p "
