...
import com.ism.job.recruitment.dto.ChartDTO;
import com.ism.job.recruitment.dto.VacancyForInterviewDTO;
import com.ism.job.recruitment.entity.Department;
...
public interface VacancyRepository extends JpaRepository<Vacancy, Long>, DataTablesRepository<Vacancy, Long> {
	...
	@Query("SELECT new com.ism.job.recruitment.dto.VacancyForInterviewDTO(v.id, v.position, v.department,v.createdDate,v.dueDate) FROM Vacancy v")
	...
	@Query("SELECT new com.ism.job.recruitment.dto.ChartDTO(s.name,COUNT(c.id)) " + "FROM Vacancy v "
	...
	@Query("SELECT new com.ism.job.recruitment.dto.ChartDTO(CONCAT('Interview',' stage ',CAST(i.stage AS string),' ', ci.status), COUNT(c.id)) FROM Vacancy v "
	...
	// to get vacancyList for interview add
	@Query("SELECT new com.ism.job.recruitment.dto.VacancyForInterviewDTO(v.id, v.position, v.department,v.createdDate,v.dueDate) FROM Vacancy v WHERE v.createdDate >= :threeMonthsAgo")
	List<VacancyForInterviewDTO> getVacanciesForInterview(LocalDate threeMonthsAgo);

	// to get vacancyList for interview add
	@Query("SELECT new com.ism.job.recruitment.dto.VacancyForInterviewDTO(v.id, v.position, v.department,v.createdDate,v.dueDate) FROM Vacancy v WHERE v.createdDate >= :threeMonthsAgo OR v.id = :vacancyId")
